...
func (r Resemble) Run(ctx context.Context, o io.WriteCloser) error {
	if r.PackageName == "" {
	...
	if r.Debug {
		return r.dynamicAssets(ctx, o)
	} else {
		return r.staticAssets(ctx, o)
	}
}
...
func (r Resemble) staticAssets(ctx context.Context, o io.WriteCloser) error {
	assets := newCollection()
	assets.RelativeBase = r.RelativeBase

	for _, aPath := range r.AssetPaths {
		err := assets.AddPath(ctx, aPath)
		if err != nil {
			return err
		}
	}
...
func (r Resemble) dynamicAssets(ctx context.Context, o io.WriteCloser) error {
